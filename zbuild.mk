...
top : pre build.o __z__loader.so

pre :
	mkdir -p $(objdir)
	mkdir -p $(sodir)

build.o : $(objdir)/build.o $(crt0) $(base_shared) $(sodir)/__z__build.so
	./ld_linux.o $(crt0) $< -v -l $(base_shared) $(sodir)/__z__build.so -o $@

$(objdir)/build.o : src/tools/build_linux.c
...
$(sodir)/__z__build.so : $(buildfile) $(base_shared) $(sodir)/__z__std.so
	./ld_linux.o $(buildfile) -l $(base_shared) $(sodir)/__z__std.so -o $@
...
$(sodir)/__z__std.so : $(objdir)/_hash_table.o $(objdir)/thread.o
	./ld_linux.o $^ -l $(sodir)/__z__os.so $(sodir)/__z__mem.so  -o $@
...
$(sodir)/__z__logger.so : $(objdir)/logger.obj
	./ld_linux.o $^ -l $(sodir)/__z__os.so -o $@
...
$(sodir)/__z__mem.so : $(objdir)/__alloc.obj
	./ld_linux.o $^ -l $(sodir)/__z__os.so $(sodir)/__z__logger.so -o $@
...
__z__loader.so : $(objdir)/loader_asm.obj $(objdir)/loader.obj
	./ld_linux.o $^ -l $(sodir)/__z__os.so $(sodir)/__z__logger.so $(sodir)/__z__mem.so -o $@
