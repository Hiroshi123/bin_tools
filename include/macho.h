...
typedef struct section_64 _section_64;
typedef struct symtab_command _symtab_command;
typedef struct dysymtab_command _dysymtab_command;
typedef struct nlist_64 _nlist_64;


typedef struct {

  // you need to retrieve 6 information from a given mach-o formatted file.  
  // the beginning and the end of text section
  // needs to be retrieved for setting the each length
  // of functions.
  // The iteration to retrieve them will be started from the end
  // of text section looking for current function on symbol table
  // to provide its length.
  const char* text_begin;
  const char* text_end;
  
  const char* reloff;
  uint32_t nreloc;  
  // symbol stubs and lazy symbol sections just like
  // plt & got on elf format.
  // They are used for detecting 
  const char* symbol_stubs_begin;
  const char* symbol_stubs_end;
  const uint32_t* lazy_symbol_begin;
  const uint32_t* lazy_symbol_end;
  
  // symbol table needs to be set 
  const _dysymtab_command* dysym_cmd_begin;
  
  const _nlist_64* sym_begin;
  const _nlist_64* sym_end;
  // dynamic symbol table is just the set of pointers towards symbol table.
  const uint32_t* dysym_begin;
  const uint32_t* dysym_end;
