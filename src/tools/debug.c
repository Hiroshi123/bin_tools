...
void do_drive(char* name) {
  ...
  // on the end of it.
  memcpy(h->begin + offset, &block, 512);
...
void th1(void* arg, uint64_t* arg2, uint64_t* arg3, uint64_t* arg4) {
  ...
  // if a thread forget letting a futex memory free, and another thread cant be
  // started. you should just register all of futexes that the thread had
  // allocated ever... as of futexes, you cannot know how many number of threads
  // are waiting on it until you wake them. if it is more than one, and keep
  // sleeping
...
int main(int argc, char** argv) {
  ...
  *CHILD_THREAD_NUM_ADDR += 1;
  printf("child thread num :: %d\n", *CHILD_THREAD_NUM_ADDR);
  uint64_t* r1 = __mmap(0, STACK_SIZE, PROT_WRITE | PROT_READ | PROT_EXEC,
  ...
  *CHILD_THREAD_NUM_ADDR += 1;
  ...
  if (!strcmp("-drive", argv[1])) {
    // bios is actually not going to be mapped but is already embedded on the
    // last part of memory(ROM). the first address which is jumped is called as
    // reset vector.
    map_bios();
    //
    do_bios();
  ...
  if (o == ELF32) {
    ...
    int i = run_through_elf_symtable3(tmp->symbol_p, sym_end,
                                      &_on_elf_symtab_callback, tmp);
    printf("ret:%x,%x,%x\n", sizeof(Elf64_Sym), tmp, sym_end);
    // beginning of elf files, heads of function table, extra data which will be
    // accumulated..
    //
    return;
    // read_elf32(h->begin, meta->begin);
