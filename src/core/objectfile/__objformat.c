...
static enum OBJECT_FORMAT check_elf(const uint32_t fd, uint8_t* e, uint32_t* size) {
  if (e[EI_MAG0] == ELFMAG0 && e[EI_MAG1] == ELFMAG1 &&
    ...
    if (class == 1) {
      ...
      if (size) *size = /*ehdr.e_ehsize + */ehdr.e_phoff + ehdr.e_phnum * sizeof(Elf32_Phdr);
      return ELF32;
    }
    else if (class == 2) {
      Elf64_Ehdr ehdr;
...
static enum OBJECT_FORMAT check_macho(const uint32_t fd, uint8_t* _p, uint32_t* size) {
  const uint32_t* p = (uint32_t*)_p;
  // what you need to do is to grab the length of header which is
  if (*p == MH_MAGIC) {
    _mach_header mh;
    if (!__os__read(fd, (void*)&mh, sizeof(_mach_header))) {}// fprintf(stderr,"file read error\n");
    *size = mh.sizeofcmds + sizeof(_mach_header);
    return MACHO32;
  } else if (*p == MH_MAGIC_64) {
    _mach_header_64 mh;
    if (!__os__read(fd, (void*)&mh, sizeof(_mach_header_64))){}// fprintf(stderr,"file read error\n");      
...
static enum OBJECT_FORMAT check_pe(const uint32_t fd, const uint8_t* _p, uint32_t* size) {
  ...
  if (*p == 0x5a4d) {
    ...
    if (!__os__read(fd, &number_of_sections, sizeof(number_of_sections))) goto error;
...
static enum OBJECT_FORMAT check_coff(const uint32_t fd, const uint8_t* _p, uint32_t* size) {
  const uint16_t* p = (uint16_t*)_p;
  switch (*p) {
  case 0x8664:
    return COFF_AMD64;
...
enum OBJECT_FORMAT __z__obj__detect_format_fname(char* fname, uint32_t* h_size) {
