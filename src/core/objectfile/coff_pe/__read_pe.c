...
uint32_t* sec_num) {
  ...
  IMAGE_NT_HEADERS32* nt_header = (IMAGE_NT_HEADERS32*)((uint8_t*)p_dos_header +
p_dos_header->e_lfanew); if (!image_base) { image_base =
...
,nt_header->OptionalHeader.AddressOfEntryPoint); printf("size of initialized
data:%d\n", nt_header->OptionalHeader.SizeOfUninitializedData); printf("size of
image:%d\n", nt_header->OptionalHeader.SizeOfImage); uint8_t data_directory_num
= 0; for (;data_directory_num<16;data_directory_num++) {
...
uint32_t* sec_num) {
  ...
  IMAGE_NT_HEADERS64* nt_header = (IMAGE_NT_HEADERS64*)((uint8_t*)p_dos_header +
p_dos_header->e_lfanew); if (image_base) { *image_base =
nt_header->OptionalHeader.ImageBase;
  }
...
(
 ...
 void** image_base
  ...
  uint32_t sec_num;
  p_guest guest_image_base = ((EXPORT(current_page_tail) + 0x1000) &
0xfffff000);
...
v_.nt_header64->OptionalHeader.DataDirectory[0].VirtualAddress; *image_base =
guest_image_base;
    ...
    *virtual_address_offset =
v_.nt_header64->OptionalHeader.DataDirectory[0].VirtualAddress; *image_base =
guest_image_base; } else { printf("error\n"); return 0;
  }
...
*addr_name); if (!strcmp(query, module_base_for_edata_section + *addr_name)) {
      ...
      printf("match:%d,%d,%x,%s\n",
             ...
             *(addr_function + *(addr_ordinal + count)),
             ...
             (uint64_t*)vv
             //module_base_for_code_section + *(addr_function + *(addr_ordinal +
count))
             );
      v1 = get_host_head(r);
...
//
/*
void iterate_import_directory(uint64_t module_base_for_idata_section, void*
iid_addr) {
  ...
  uint64_t* int_entry = module_base_for_idata_section +
iid->u.OriginalFirstThunk; uint64_t* iat_entry = module_base_for_idata_section +
iid->FirstThunk; int fd = open(filename, O_RDONLY); uint32_t h_size; enum
OBJECT_FORMAT res = detect_format(fd, &h_size); struct stat stbuf; if (fstat(fd,
&stbuf) == -1) { close(fd); return;
  }
...
edata_addr); char first = 1; for (;*int_entry;int_entry++, iat_entry++) {
    printf("!!!%x,%s\n",int_entry,module_base_for_idata_section + *int_entry +
2); char forward;
...
/* // whatever it is set on base + iat, all you just do is to just fill the
 * value */
...
static void do_image_directory_export(void* arg1, void* arg2) {
  ...
  IMAGE_EXPORT_DIRECTORY* ied =
      (IMAGE_EXPORT_DIRECTORY*)(_dso->base + id->VirtualAddress);
  ...
  uint32_t* addr_name = _dso->base + ied->AddressOfNames;
  uint32_t* addr_name_end = addr_name + ied->NumberOfNames;
  uint32_t* addr_function = _dso->base + ied->AddressOfFunctions;
  uint32_t* addr_function_end = addr_function + ied->NumberOfFunctions;
  uint16_t* addr_ordinal = _dso->base + ied->AddressOfNameOrdinals;
...
static void do_image_directory_import(void* arg1, void* arg2) {
  ...
  IMAGE_IMPORT_DESCRIPTOR* iid =
      (IMAGE_IMPORT_DESCRIPTOR*)(_dso->base + id->VirtualAddress);
  ...
  /* #ifndef DEBUG */
  /*   printf("------------image import directory--------------\n"); */
  /*   printf("image import descriptor head
   * addr:%lx\n",&iid->u.OriginalFirstThunk); */
  /*   printf("int head addr:%x\n",iid->u.OriginalFirstThunk); */
  /*   printf("time date stamp:%x\n",iid->TimeDateStamp); */
  /*   printf("name:%s\n",filename); */
...
char* __z__obj__load_pe(char* name, void* out) {
  // void* head = __z__mem__alloc_file(name);
  info_on_pe* info = out;
  ...
  uint16_t* nt_optional_header_head = (uint16_t*)(ifh + 0x1);
  IMAGE_OPTIONAL_HEADER32* opt = nt_optional_header_head;
  ...
  info->fd = fd;
  // CAUTION
  // not exactly .text section
  info->base_of_code = opt->BaseOfCode;
  info->size_of_code = opt->SizeOfCode;
