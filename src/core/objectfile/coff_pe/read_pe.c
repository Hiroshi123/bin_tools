...
uint64_t map_pe(p_host p_dos_header, p_host section_head, uint32_t section_num,
  ...
  memcpy(h1->begin, p_dos_header, len);
#ifndef DEBUG
  ...
  for (; s < section_end; s++) {
    ...
    void* dst = memcpy(h2->begin, (uint8_t*)p_dos_header + s->PointerToRawData,
                       s->SizeOfRawData);
    if (image_base + s->VirtualAddress + s->SizeOfRawData >
        EXPORT(current_page_tail)) {
      EXPORT(current_page_tail) =
          image_base + s->VirtualAddress + s->SizeOfRawData;
    }
#ifndef DEBUG
    printf("---------------section info-----------------------\n");
    printf("name:%s\n", s->Name);
...
p_host retrive_nt_header_32(IMAGE_DOS_HEADER* p_dos_header, p_guest* image_base,
                            uint32_t* sec_num) {
  IMAGE_NT_HEADERS32* nt_header =
      (IMAGE_NT_HEADERS32*)((uint8_t*)p_dos_header + p_dos_header->e_lfanew);
  if (!image_base) {
  ...
  *sec_num = nt_header->FileHeader.NumberOfSections;
  ...
  for (; data_directory_num < 16; data_directory_num++) {
    ...
    printf("%d,VirtualAddress:%x\tSize:%d\n", data_directory_num,
           nt_header->OptionalHeader.DataDirectory[data_directory_num]
               .VirtualAddress,
...
p_host retrive_nt_header_64(IMAGE_DOS_HEADER* p_dos_header, p_guest* image_base,
                            uint32_t* sec_num) {
  IMAGE_NT_HEADERS64* nt_header =
      (IMAGE_NT_HEADERS64*)((uint8_t*)p_dos_header + p_dos_header->e_lfanew);
  if (image_base) {
    *image_base = nt_header->OptionalHeader.ImageBase;
  }
...
void* map_pe_for_check_export(uint64_t dll_name_addr, void* head,
  ...
  p_guest guest_image_base =
      ((EXPORT(current_page_tail) + 0x1000) & 0xfffff000);
...
// return address will refer the string which contans characters of DLL and
// function having . in between the two. if not forwarded, it will refer the
// head of address of the function.

// you can set the value on import address table of caller dll or exe.
// it should also be provided as guest address as it is wierd if every rest of
// address is represented as guest address but if the only IAT holds host
// address.

p_guest find_f_from_export_directory(uint32_t edata_virtual_address_offset,
                                     void* edata_addr, char* query,
                                     char* forward) {
  ...
  uint32_t* addr_name = module_base_for_edata_section + ied->AddressOfNames;
  ...
  for (; addr_name != addr_name_end; addr_name++, count++) {
    ...
    /* 	   module_base_for_edata_section,module_base_for_edata_section +
     * *addr_name); */
    if (!strcmp(query, module_base_for_edata_section + *addr_name)) {
       ...
       * *\/ */
      /* 	     (uint64_t*)vv */
      /* 	     //module_base_for_code_section + *(addr_function +
       * *(addr_ordinal + count)) */
      /* 	     ); */
      v1 = get_host_head(r);
...
// before mapping idata section or rdata which are going to substitute where
// idata sits,
//
void iterate_import_directory(uint64_t module_base_for_idata_section,
                              void* iid_addr) {
  ...
  uint64_t* int_entry =
      module_base_for_idata_section + iid->u.OriginalFirstThunk;
  ...
  p_host image_base;
  void* edata_addr = map_pe_for_check_export(
      filename, h->begin, &edata_virtual_address_offset, &image_base);
  printf("base:%lx,%lx,%lx\n", image_base, edata_virtual_address_offset,
         edata_addr);
  char first = 1;
  for (; *int_entry; int_entry++, iat_entry++) {
    printf("!!!%x,%s\n", int_entry,
           module_base_for_idata_section + *int_entry + 2);
    char forward;
    // you need to add HINT which is 2byte on image import by name struct
    void* query = module_base_for_idata_section + *int_entry + 2;
    if (first) {
      p_guest r = find_f_from_export_directory(edata_virtual_address_offset,
                                               edata_addr, query, &forward);
      p_host* vv;
      printf("r:%x\n", r);
      ...
      // void* base_addr = (uint64_t)edata_addr -
       ...
       */
      /* // whatever it is set on base + iat, all you just do is to just fill
       * the value */
      /* // on it.[NOTE this is .rdata section..] */
      /* // or just simply iat entry itself which was set on import address
       * table???       */
  ...
  // 7.fill the retrived value from export table in import address table
  // accordingly.
...
uint32_t load_pe(void* head) {
  ...
  if (*nt_optional_header_head == 0x10b) {
    section_head =
        retrive_nt_header_32(dos_header, &guest_image_base, &sec_num);
    ...
    import_addr = host_nt_header.nt_header32->OptionalHeader.ImageBase +
                  host_nt_header.nt_header32->OptionalHeader.DataDirectory[1]
                      .VirtualAddress;
    distance_from_image_base =
        host_nt_header.nt_header32->OptionalHeader.DataDirectory[1]
            .VirtualAddress;
    start_addr = host_nt_header.nt_header32->OptionalHeader.ImageBase +
    ...
    section_head =
        retrive_nt_header_64(dos_header, &guest_image_base, &sec_num);
    map_pe(dos_header, section_head, sec_num, guest_image_base, 0);
    // guest_image_base = map_pe64(dos_header, NULL, 0);
    get_diff_host_guest_addr_(guest_image_base, &tmp);
    host_nt_header.nt_header64 = (IMAGE_NT_HEADERS64*)((uint8_t*)tmp + lfanew);
    import_addr = host_nt_header.nt_header64->OptionalHeader.ImageBase +
                  host_nt_header.nt_header64->OptionalHeader.DataDirectory[1]
                      .VirtualAddress;
    distance_from_image_base =
        host_nt_header.nt_header64->OptionalHeader.DataDirectory[1]
            .VirtualAddress;
    start_addr = host_nt_header.nt_header64->OptionalHeader.ImageBase +
  ...
  iterate_import_directory(module_base_for_idata_section,
  ...
  // (IMAGE_IMPORT_DESCRIPTOR*)idata_section_head;

  /* /\* *iid++; *\/ */
  /* iterate_import_directory(distance_from_image_base, iid); */

  // uint8_t* module_base_for_idata_section = (uint8_t*)idata_section_head -
  // distance_from_image_base;
}
...
// you need to provide a guest address which tells you if it is within current
// IAT.
...
p_host EXPORT(find_f_addr(p_guest rip, p_guest query)) {
  ...
  printf("%x,%x,%x,%s\n", tmp, *(uint64_t*)tmp, 0x400000 + *(uint64_t*)tmp,
         (uint8_t*)f_name);

  // you also need to reach the dll name.
  p_host dll_name = get_dll_name(iid_v_addr, idata_section, f_name);

  // After reaching dll, you should search the corresponding page among ever
  // mapped page given the dll name
