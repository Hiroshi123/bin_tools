...
p_guest load_elf32(uint8_t *page_head, info_on_elf32 *_e) {
  ...
  for (; phdr != phdr_end; phdr++) {
    ...
    memcpy(h1->begin, page_head + phdr->p_offset, phdr->p_filesz);
...
void read_elf32(const uint8_t *head, info_on_elf32 *_e) {
  ...
  _e->shdr_tail =
      (Elf32_Shdr *)((size_t)_e->shdr_head + h->e_shentsize * h->e_shnum);
  _e->shstr_offset =
      (char *)((Elf32_Shdr *)((size_t)_e->shdr_head +
                              (size_t)(h->e_shentsize * h->e_shstrndx)))
          ->sh_offset +
      (size_t)head;
  Elf32_Shdr *shdr;
  char *sh_name;
...
char read_elf64(const char *const page_for_elf, info_on_elf *_e) {
  ...
  _e->shdr_tail = (Elf64_Shdr *)((size_t)_e->shdr_head +
  ...
  for (shdr = _e->shdr_head; shdr != _e->shdr_tail; shdr++, i++) {
    ...
    if (shdr->sh_type == SHT_SYMTAB) {
      ...
      if (!strcmp(sh_name, ".rela.text")) {
        _e->rela_text_p =
            (Elf64_Rela *)(shdr->sh_offset + (size_t)page_for_elf);
  ...
  if ((mark & 0b111111) != 0b111111) {
    ...
    if ((mark & 1) != 1) {
      if ((mark & 2) != 2) {
        printf(
            "error:either dynamic or normal symbol table must be on this "
            "elf file\n");
        return 0;
      }
...
void *get_name_of_f_on_elf32(p_guest f_addr, info_on_elf32 *_e) {
  ...
  const Elf32_Sym *symbol_end =
      (Elf32_Sym *)((size_t)symbol_begin + (size_t)_e->symbol_size);
...
char *get_addr_of_f(const char *fname, info_on_elf *_e) {
  ...
  const Elf64_Sym *symbol_end =
      (Elf64_Sym *)((size_t)symbol_begin + (size_t)_e->symbol_size);
...
char *do_relocs(const char *query, info_on_elf *_e1, info_on_elf *_e2) {
  ...
  const Elf64_Sym *symbol_end =
      (Elf64_Sym *)((size_t)symbol_begin + (size_t)_e1->symbol_size);
  Elf64_Sym *p = (Elf64_Sym *)symbol_begin;
  char *fname;
    ...
    if (ELF64_ST_BIND(p->st_info) == STB_GLOBAL &&
      ...
      if (p->st_shndx == _e1->text_shndx) {
        ...
        if (!memcmp(fname, query, strlen(query))) {
          ...
          // section.
          for (; r != rela_end; r++) {
            // if the offset of the entry is within a range of the head & tail
            // of a function which is searched, then the relocation entry must
            // be a callee from the function.
            if (caller_head < r->r_offset && r->r_offset < caller_tail) {
              char *callee_name =
                  _e1->str_p +
                  ((symbol_begin + ELF64_R_SYM(r->r_info))->st_name);
              uint32_t *caller_arg_addr =
                  (uint32_t *)(_e1->text_p + r->r_offset);
