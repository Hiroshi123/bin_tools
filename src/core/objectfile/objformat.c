...
enum OBJECT_FORMAT check_elf(const uint32_t fd, uint8_t* e, uint32_t* size) {
  if (e[EI_MAG0] == ELFMAG0 && e[EI_MAG1] == ELFMAG1 && e[EI_MAG2] == ELFMAG2 &&
    ...
    if (class == 1) {
      ...
      *size =
          /*ehdr.e_ehsize + */ ehdr.e_phoff + ehdr.e_phnum * sizeof(Elf32_Phdr);
      return ELF32;
    } else if (class == 2) {
      Elf64_Ehdr ehdr;
      if (!read(fd, &ehdr, sizeof(ehdr))) goto error;
...
enum OBJECT_FORMAT check_macho(const uint32_t fd, const uint32_t* p,
                               uint32_t* size) {
  // what you need to do is to grab the length of header which is
  if (*p == MH_MAGIC) {
    _mach_header mh;
    if (!read(fd, mh, sizeof(_mach_header)))
      fprintf(stderr, "file read error\n");
    *size = mh.sizeofcmds + sizeof(_mach_header);
    return MACHO32;
  } else if (*p == MH_MAGIC_64) {
    _mach_header_64 mh;
    if (!read(fd, mh, sizeof(_mach_header_64)))
      fprintf(stderr, "file read error\n");
...
enum OBJECT_FORMAT check_pe(const uint32_t fd, const uint16_t* p,
                            uint32_t* size) {
  if (*p == 0x5a4d) {
