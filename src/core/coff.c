...
char read_coff(const char *const begin,
  ...
  for (;s!=e->sec_end;s++) {
    ...
    if (!strcmp(s->Name, ".text") && !filled) {
      ...
      e->text_reloc_end = e->text_reloc_begin + sizeof(relocation_entry) * (s->NumberOfRelocations);      
    }
  ...
  for (;is!=e->sym_end;is++) {    
    name = (*(uint32_t*)is == 0) ?
      (char*) ((size_t)str_begin+*((uint32_t*)is+1)) : is->N.ShortName;
#ifdef DEBUG
    printf("val:%x\n",is->Value);
    printf("%s\n",name);
    printf("value:%x\n", is->Value);
...
char* do_reloc_coff(/*const char* query,*/ info_on_coff* e1, info_on_coff* e2) {
  ...
  for (;r!=e1->text_reloc_end;r++) {
    ...
    char* fname = (*(uint32_t*)caller_sym == 0) ?
      (char*) ((size_t)e1->str_begin+*((uint32_t*)caller_sym+1)) : caller_sym->N.ShortName;
#ifdef DEBUG
    printf("%s\n",fname);
#endif
    char* n;
    ...
    for (;sym!=e1->sym_end;sym++) {
      n = (*(uint32_t*)sym == 0) ?
	(char*) ((size_t)e1->str_begin+*((uint32_t*)sym+1)) : sym->N.ShortName;    
      if (!strcmp(fname, n)) {
	...
	*caller_arg_addr = (uint32_t)(callee_addr - caller_next_addr);
    ...
    for (;sym!=e2->sym_end;sym++) {
      n = (*(uint32_t*)sym == 0) ?
	(char*) ((size_t)e2->str_begin+*((uint32_t*)sym+1)) : sym->N.ShortName;
      if (!strcmp(fname, n)) {
...
char* find_test_(char* query, info_on_coff* e) {
  ...
  for (;s!=e->sym_end;s++) {
    n = (*(uint32_t*)s == 0) ?
      (char*) ((size_t)e->str_begin+*((uint32_t*)s+1)) : s->N.ShortName;    
